  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        {/* Card 1: Explanation */}
        <div className="border border-brand-light/8 bg-brand-light/4 p-6 md:p-8 space-y-4">
        ...
        <div className="border border-brand-light/8 bg-brand-light/4 p-6 md:p-8 space-y-6">
          <h4 className="text-xs font-bold tracking-widest text-brand-light/60 uppercase select-none">
            Practice MCQs
          </h4>
                ...
                return (
                  <div key={qIdx} className={`space-y-4 ${qIdx > 0 ? "pt-8" : ""}`}>
                    <div className="flex items-start space-x-3">
                      ...
                      <p className="font-sans text-brand-light font-medium text-base">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-8">
                      {["A", "B", "C", "D"].map((letter) => {
                        ...
                        let btnStyle =
                          "border border-brand-light/8 bg-brand-light/4 text-brand-light hover:border-brand-light/20 transition-all text-left p-4 flex items-center space-x-3 w-full rounded-none";

                        if (state.revealed) {
                          if (isCorrect) {
                            // Correct answer highlighted as a solid #F7F8FC (brand-light) block with black (brand-dark) text
                            btnStyle =
                              "bg-brand-light text-brand-dark font-bold border-brand-light text-left p-4 flex items-center space-x-3 w-full rounded-none select-none cursor-default";
                          } else if (isSelected) {
                            // Selected incorrect option shows with red border
                            btnStyle =
                              "border border-red-500 text-brand-light/70 bg-transparent text-left p-4 flex items-center space-x-3 w-full rounded-none select-none cursor-default";
                          } else {
                            // Other options become secondary/disabled
                            btnStyle =
                              "border border-brand-light/4 text-brand-light/35 bg-transparent text-left p-4 flex items-center space-x-3 w-full rounded-none select-none cursor-default";
                          }
                        }
                    ...
                    {!state.revealed && !isHistoryView && (
                      <div className="pl-8">
                        <button
                          ...
                          className="text-xs text-brand-light/40 hover:text-brand-light/70 underline underline-offset-4 cursor-pointer select-none"
                        >
                          Reveal Answer
                        </button>
                      </div>
        ...
        <div className="border border-brand-light/8 bg-brand-light/4 p-6 md:p-8 space-y-4">
          ...
          <p className="text-brand-light/80 text-base leading-relaxed italic">
